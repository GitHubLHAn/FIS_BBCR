...
/*Declare Structs--------------------------------------------------------------------------*/
	typedef struct{
		...
		volatile uint32_t PulseSetPoint, PulseCurrent;
		volatile uint8_t currentPos, targetPos;
		
		volatile bool toRunning, toZero, toConfirmSetting, toSetting, toRunFree, toTargetPos, toAutoRun;
...
/*Declare Functions------------------------------------------------------------------------*/
	...
	void Command_toSetDegree(Motor_t *pM, uint8_t _Pos, float degree);
